 ...
 *
 */
public class CreatePublicationWebScript extends AbstractCmsCommonWebScript {
    ...
    protected void executeService(WebScriptRequest webScriptRequest, WebScriptResponse webScriptResponse) {
        try {
            ...
            AbstractPublicationBaseCommonService pubCommonSrvc = (AbstractPublicationBaseCommonService) publicationServiceFactory
                    .getPublicationService(publicationServiceMap.get(docType));

            // Invoke the doctype specific service which creates the document
            TmngAlfResponse response = pubCommonSrvc.create(cmsId, fileName, ContentItem.getInstance(content), null);
