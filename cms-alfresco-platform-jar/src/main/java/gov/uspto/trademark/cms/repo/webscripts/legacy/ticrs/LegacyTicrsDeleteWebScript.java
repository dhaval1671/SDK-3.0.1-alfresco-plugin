 ...
 *
 */
public class LegacyTicrsDeleteWebScript extends AbstractCmsCommonWebScript {

    private static final String REF_LINK = "ref-link";
    ...
    protected void executeService(WebScriptRequest webScriptRequest, WebScriptResponse webScriptResponse) {
        try {
            ...
            String docType = webScriptRequest.getParameter(DOC_TYPE);
            String docTypeToPass = docType;
            if (StringUtils.isBlank(docType)) {
                docType = LegacyTicrsDeleteWebScript.REF_LINK;
                docTypeToPass = null;
            }
            ...
            TmngAlfResponse response = implSrvc.ticrsAdminDelete(serialNumber, fileName, docTypeToPass);
            ...
            throw new TmngCmsException.CMSWebScriptException(HttpStatus.valueOf(e.getStatus()), e.getMessage(), e);
        } catch (Exception e) {
            throw new TmngCmsException.CMSWebScriptException(HttpStatus.BAD_REQUEST, e.getMessage(), e);
        }
