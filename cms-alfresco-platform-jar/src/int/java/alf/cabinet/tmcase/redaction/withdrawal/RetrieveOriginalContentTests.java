...
public class RetrieveOriginalContentTests extends RedactionWithdrawalBaseTest {
	 ...
	 */
	@Test
	public void testRetrieveOriginalWithdrawalContent() {
		createOriginalWithdrawal();
		createRedactedVersionofOriginalWithdrawal();
		retrieveOriginalWithdrawalContent();
		retrieveOriginalWithdrawalContentWithInvalidAccessLevel();
	}
	...
	private void createRedactedVersionofOriginalWithdrawal() {
		...
		redactedWithdrawalParam.put(ParameterKeys.CONTENT_ATTACHEMENT.toString(), RedactionWithdrawalBaseTest.CONTENT_CREATE_REDACTED_RESPONSE);
		ClientResponse redactedWithdrawal = updateDocument(redactedWithdrawalParam);
        //String str = redactedWithdrawal.getEntity(String.class);
	 ...
	 */
	private void retrieveOriginalWithdrawalContent() {
		UrlInputDto urlInput = new UrlInputDto(TradeMarkDocumentTypes.TYPE_WITHDRAWAL.getAlfrescoTypeName());
		urlInput.setSerialNumber(ParameterValues.VALUE_SERIAL_77777778_NUMBER.toString());
		urlInput.setFileName(RedactionWithdrawalBaseTest.VALUE_WITHDRAWAL_FILE_NAME_TWO);
		urlInput.setAccessLevel(AccessLevel.INTERNAL.toString());
		String RedactedRetrieveWithdrawalContent = CaseRetrieveContentUrl.getRedactedRetrieveContentFlavourOriginal(urlInput);
    ...
    private void retrieveOriginalWithdrawalContentWithInvalidAccessLevel() {
        UrlInputDto urlInput = new UrlInputDto(TradeMarkDocumentTypes.TYPE_WITHDRAWAL.getAlfrescoTypeName());
        urlInput.setSerialNumber(ParameterValues.VALUE_SERIAL_77777778_NUMBER.toString());
        urlInput.setFileName(RedactionWithdrawalBaseTest.VALUE_WITHDRAWAL_FILE_NAME_TWO);
