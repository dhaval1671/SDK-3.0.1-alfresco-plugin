...
/**
 * The Class RedactedRetrieveEvidenceMetadataTests.
 *
 * @author Sanjay Tank {linkedin.com/in/sanjaytaunk}
 */

public class RedactedRetrieveEvidenceMetadataTests extends RedactionEvidenceBaseTest {
    ...
    public static void setUp() {
        ...
        //createEvidenceDocVersionStackWithOriginalVersionOnTop(serialNumber78);
        //createEvidenceDocVersionStackWithRedactedVersionOnTop(serialNumber79);        
    }
    ...
    /**
     * Test redacted retrieve metadata default flavour with redacted version on top.
     */
    @Test
    public void testRedactedRetrieveMetadataDefaultFlavourWithRedactedVersionOnTop() {
        ...
        String haystack = str;
        //check for redacted version which should be 1.2 and Original version is on the top ie 1.3.
        ...
        String modificationTime = "\"modifiedByUserId\"(.*?)\"redactedEvidence\"";
    ...
    /**
     * Test redacted retrieve metadata with flavour as original with redacted version on top.
     */
    @Test
    public void testRedactedRetrieveMetadataWithFlavourAsOriginalWithRedactedVersionOnTop() {
        ...
        urlInput.setAccessLevel(AccessLevel.RESTRICTED.toString());
        String redactedRetrieveResponseMetadata = CaseRetrieveMetadataUrl.getRedactedRetrieveMetadataFlavourOriginal(urlInput);
        //System.out.println(redactedRetrieveResponseMetadata);
        WebResource webResource = client.resource(redactedRetrieveResponseMetadata);
        ClientResponse response = webResource.type(MediaType.MULTIPART_FORM_DATA_TYPE).accept(MediaType.APPLICATION_JSON_TYPE).get(ClientResponse.class);
        assertEquals(200, response.getStatus());
        ...
        //System.out.println(str);
        String haystack = str;
        //check for redacted version which should be 1.1 and Original version is on the bottom ie 1.0.
        String creationTime = "\"version\"(.*?)\"1.0\"";
        assertTrue(containsOneOrMoreOccurencesOfProvidedStringOfGivenRegEx( haystack, creationTime));
        //check for modifiedByUserId it should be 'redactedResponse'
        String modificationTime = "\"modifiedByUserId\"(.*?)\"originalEvidence\"";
        assertTrue(containsOneOrMoreOccurencesOfProvidedStringOfGivenRegEx( haystack, modificationTime));
        String docType = "\"documentType\"(.*?)\""+ Evidence.TYPE + "\"";
        assertTrue(containsOneOrMoreOccurencesOfProvidedStringOfGivenRegEx( haystack, docType));         
    }      
    ...
    public void testRedactedRetrieveMetadataWithFlavourAsOriginalWithOriginalVersionOnTop() {
        ...
        String haystack = str;
        //check for redacted version which should be 1.1 and Original version is on the bottom ie 1.0.
        String creationTime = "\"version\"(.*?)\"1.3\"";
        assertTrue(containsOneOrMoreOccurencesOfProvidedStringOfGivenRegEx( haystack, creationTime));
        //check for modifiedByUserId it should be 'redactedResponse'
        String modificationTime = "\"modifiedByUserId\"(.*?)\"originalEvidence\"";
        assertTrue(containsOneOrMoreOccurencesOfProvidedStringOfGivenRegEx( haystack, modificationTime));
